#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""속도별 안정성 스윕. +500펄스 이동을 여러 속도로. 반대방향(폭주) 감지시 즉시정지."""
...
def cfg(pos):
    ...
    for idx, sub, val, f in [
        ...
        (0x6060, 0, 1, "b"), (0x6083, 0, 50000, "<I"), (0x6084, 0, 50000, "<I"),
    ]:
...
def trymove(speed, cmd=500):
    ...
    for i in range(250):
        ...
        if d < -50:                      # 반대방향 = 폭주
            for _ in range(6):
                io(0x00, 0)
            return f"⚠️폭주(반대방향 {d:+d}) 정지"
        if abs(d) > cmd + 300:           # 오버슈트 폭주
            for _ in range(6):
                io(0x00, 0)
            return f"⚠️폭주(오버슈트 {d:+d}) 정지"
        if sw & 0x0008:
            return f"폴트 (이동 {d:+d})"
        if abs(apos - (start + cmd)) < 20:
            pump(0x00, 0, 8)
            return f"✅도달 (이동 {d:+d}/{cmd}, 피크{peak})"
        time.sleep(0.005)
    pump(0x00, 0, 8)
    return f"미완 (이동 {io(0x0F,0)[1]-start:+d}/{cmd})"
...
def main():
    ...
    print("속도별 +500펄스 이동:")
    for spd in [2000, 4000, 6000, 8000, 10000, 15000]:
        print(f"  speed={spd:6d}: {trymove(spd)}")
        pump(0x00, 0, 5)
